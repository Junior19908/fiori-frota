sap.ui.define([
  ...
  function headerIndex(headers) {
    ...
    return {
      ...
      Data: find("data inicio movimento", "data", "data combustivel", "data combustível"),
      Hora: find("hora inicio movimento", "hora", "hora combustivel", "hora combustível"),
      Hodometro: find("hodometro", "hodômetro", "quilometragem", "km"),
      Horimetro: find("horimetro", "horímetro"),
  ...
  function parseNumber(v) {
    ...
    // Caso brasileiro comum: 1.234,56
  ...
  function mkId(prefix, counters, dateObj) {
    if (!dateObj) return null;
    var y = dateObj.getUTCFullYear();
    var m = String(dateObj.getUTCMonth() + 1).padStart(2, "0");
    var key = String(y) + m;
    counters[key] = (counters[key] || 0) + 1;
    return prefix + "-" + key + "-" + String(counters[key]).padStart(6, "0");
  }
  ...
  function findHeaderRow(rows) {
    ...
    for (var i = 0; i < Math.min(12, rows.length); i++) {
      var r = rows[i] || [];
      var has = wanted.every(function (w) {
        return r.some(function (c) { return String(c || "").toLowerCase() === w.toLowerCase(); });
      });
      if (has) return i;
    }
    return 0;
  ...
  return Controller.extend("com.skysinc.frota.frota.controller.ImportAbastecimentos", {
    ...
    onInit: function () {
      this._raw = [];
      this._events = [];
      this._progressModel = new JSONModel({
        total: 0, current: 0, created: 0, updated: 0, skipped: 0,
        errors: 0, percent: 0, text: ""
      });
      try { this._showErrorsDialog = this._showErrorsDialogCSV.bind(this); } catch (_) {}
    },

    onNavBack: function () {
      try { this.getOwnerComponent().getRouter().navTo("settings"); } catch (_) {}
    },
    ...
    _ensureProgressDialog: function (autoOpen) {
      ...
      sap.ui.require([
        "sap/m/Dialog", "sap/m/ProgressIndicator", "sap/m/Text",
        "sap/m/Button", "sap/m/VBox"
      ], function (Dialog, ProgressIndicator, Text, Button, VBox) {
        ...
        var stats = new Text({
          text: "{= 'Criadas: ' + ${progress>/created} + '  |  Atualizadas: ' + ${progress>/updated} + '  |  Ignoradas: ' + ${progress>/skipped} + '  |  Erros: ' + ${progress>/errors} }"
        });
        ...
        var dlg = new Dialog({
          ...
          afterClose: function () {
            ...
            try { that._dlgProgress = null; } catch (_) {}
          }
        });
    ...
    _openProgress: function (total) {
      this._progressModel.setData({
        total: total, current: 0, created: 0, updated: 0, skipped: 0,
        errors: 0, percent: 0, text: "0% (0/" + total + ")"
      });
    ...
    _finishProgress: function () {
      ...
      if (this._dlgProgress) {
        ...
        setTimeout(function () {
          try { this._dlgProgress.close(); } catch (_) {}
        }.bind(this), 800);
      }
    },

    _errorsToCSV: function (errors) {
      var lines = ["linha,motivo"];
      (errors || []).forEach(function (e) {
        lines.push([String(e.line || ""), '"' + String(e.reason || "").replace(/"/g, '""') + '"'].join(","));
      });
      return lines.join("\r\n");
    },
    ...
    _downloadErrorsCSV: function (errors, name) {
      try {
        ...
        a.href = url;
        a.download = name || "erros_importacao.csv";
    ...
    _showErrorsDialogCSV: function (errors, title) {
      ...
      sap.ui.require(["sap/m/Dialog", "sap/m/List", "sap/m/StandardListItem", "sap/m/Button", "sap/m/Text"],
        function (Dialog, List, SLI, Button, Text) {
          var list = new List({ inset: false, growing: true });
          top.forEach(function (e) {
            list.addItem(new SLI({
              title: "Linha " + String(e.line || "?"),
              description: String(e.reason || "Erro"),
              type: "Inactive"
            }));
          });
          ...
          var dlg = new Dialog({
            ...
            beginButton: new Button({
              text: "Baixar CSV",
              type: "Emphasized",
              press: function () { that._downloadErrorsCSV(errors, "erros_importacao.csv"); }
            }),
    ...
    _ensureXLSX: function () {
      return new Promise(function (resolve, reject) {
        function shape(mod) {
          var c = mod && (mod.default || mod);
          return (c && c.read && c.utils) ? c : null;
        }
        try {
        ...
        function next() {
          import("https://cdn.sheetjs.com/xlsx-0.20.2/package/xlsx.mjs")
            .then(function (esm) {
              var s = shape(esm);
              if (s) return resolve(s);
              fallbackGlobal();
        ...
        function fallbackGlobal() {
          if (window.XLSX && window.XLSX.read) return resolve(window.XLSX);
          var url = "https://cdn.sheetjs.com/xlsx-0.20.2/package/xlsx.full.min.js";
          // jQuery.sap.includeScript segue disponível em 1.139
          jQuery.sap.includeScript(url, "sheetjs-xlsx",
            function () { if (window.XLSX && window.XLSX.read) resolve(window.XLSX); else reject(new Error("Falha XLSX")); },
            function (e) { reject(e || new Error("Falha XLSX")); }
          );
        }
      });
    },
      ...
      this._ensureXLSX().then(function (XLSX) {
        ...
        reader.onload = function (e) {
          try {
            ...
            if (readErrors.length) that._showErrorsDialog(readErrors, "Erros ao ler o Excel");
          } catch (err) {
    ...
    _renderPreview: function () {
      ...
      if (txt) txt.setText("Linhas lidas: " + rows.length + ". Exibindo primeiras " + Math.min(rows.length, 50) + ".");
    },

    onProcess: function () {
      var ym = String(this.byId("inpMonth").getValue() || "").trim();
      var m = ym.match(/^(\d{4})-(\d{2})$/);
      if (!m) { MessageToast.show("Informe Mês no formato YYYY-MM."); return; }

      var y = Number(m[1]), mm = Number(m[2]);
      var precoFallback = parseNumber(this.byId("inpPreco").getValue());

      var raw = this._raw || [];
      if (!raw.length) { MessageToast.show("Carregue um Excel primeiro."); return; }

      // filtra pelo mês informado
      var filtered = raw.filter(function (r) {
        if (!r.dataObj) return false;
        return (r.dataObj.getUTCFullYear() === y && (r.dataObj.getUTCMonth() + 1) === mm);
      });
      ...
      // Gera JSON local e oferece download (sem Firestore aqui)
      try {
        var seq = 0; var counters = {};
        var resultMap = {}; var prevIndex = {};
        ...
        for (var i = 0; i < filtered.length; i++) {
          ...
          try {
            seq += 1;
            var idEvt = mkId("A", counters, r.dataObj);
            var evt = {
        ...
        var url = URL.createObjectURL(blob);
        var a = document.createElement("a");
        var fname = "abastecimentos-" + String(y) + "-" + String(mm).padStart(2, "0") + ".json";
        a.href = url; a.download = fname; document.body.appendChild(a); a.click();
        setTimeout(function () { document.body.removeChild(a); URL.revokeObjectURL(url); }, 0);

        MessageToast.show(
          "JSON gerado: " + fname +
          " (Criadas " + created + ", Ignoradas " + skipped +
          (warns ? ", Avisos (sem hora) " + warns : "") + ")"
        );
        ...
        // eslint-disable-next-line no-console
        console.error(e);
        MessageToast.show("Falha ao gerar o arquivo JSON.");
        return;
      }
    }
